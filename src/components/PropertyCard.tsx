...
import { formatPrice, getLocationString, getYouTubeVideoId } from '../lib/utils'
...
export default function PropertyCard({ property }: PropertyCardProps) {
  ...
  const videoId = getYouTubeVideoId(property.youtube_url)
          ...
          {videoId && property.youtube_url && (
            <div className="absolute inset-0 z-[8] flex items-center justify-center pointer-events-none">
              <button
                ...
                className="pointer-events-auto p-4 cursor-pointer focus:outline-none"
                aria-label={`Play video for ${property.title}`}
              >
                <YouTubePlayOverlay size={56} />
              </button>
            </div>
          )}
        </div>
